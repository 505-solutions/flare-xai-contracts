...
contract MainContractTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy the MainContract contract
        mainContract = new MainContract(address(flareVtpm), address(wlfrToken));
    ...
    function testActivateWallet() public {
        ...
        // Mock the attestation verification
        bytes memory header = ""; // Mock header
        bytes memory payload = ""; // Mock payload
        bytes memory signature = ""; // Mock signature

        // Assume the attestation verification is successful
        vm.mockCall(
            address(flareVtpm),
            abi.encodeWithSelector(flareVtpm.verifyAndAttest.selector, header, payload, signature),
            abi.encode(true)
        );

        // Activate the wallet
        mainContract.activateWallet(header, payload, signature);
